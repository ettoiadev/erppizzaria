...
export function CheckoutForm({ onSubmit, isLoading, userId }: CheckoutFormProps) {
    ...
    async function loadUserData() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData(prev => ({
            ...prev,
            name: userData.user?.name || userData.user?.full_name || user.full_name || "",
            phone: userData.user?.phone || (user as any).phone || "",
          }))
        } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const orderData = {
      ...
      delivery_phone: formData.phone || (user as any)?.phone || ""
    }
