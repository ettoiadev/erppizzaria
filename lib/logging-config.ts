...
export function getLoggingConfig(): LoggingConfig {
  ...
  // Override específicos do Supabase
  if (process.env.ENABLE_QUERY_LOGS !== undefined) {
    config.contexts.supabase.logQueries = process.env.ENABLE_QUERY_LOGS === 'true'
    config.contexts.api.logQueries = process.env.ENABLE_QUERY_LOGS === 'true'
  }
  
  if (process.env.ENABLE_SLOW_QUERY_LOGS !== undefined) {
    config.contexts.supabase.logSlowQueries = process.env.ENABLE_SLOW_QUERY_LOGS === 'true'
  }
  
  if (process.env.SLOW_QUERY_THRESHOLD) {
    const threshold = parseInt(process.env.SLOW_QUERY_THRESHOLD)
    if (!isNaN(threshold) && threshold > 0) {
      config.contexts.supabase.slowQueryThreshold = threshold
    }
  }
