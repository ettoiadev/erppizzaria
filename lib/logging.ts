// Sistema de logging avançado para Supabase e Vercel
import { logger as debugLogger } from './debug-utils'
...
class AdvancedLogger {
  ...
  private async sendToExternalService(entry: LogEntry): Promise<void> {
    ...
    try {
      ...
      if (this.isProduction) {
        ...
        // await sendToVercelAnalytics(entry)
  ...
  public supabase = {
    query: (message: string, data?: any, metadata?: Partial<LogEntry>) => {
      this.debug('supabase', `Query: ${message}`, data, metadata)
    },
    error: (message: string, error: Error, data?: any, metadata?: Partial<LogEntry>) => {
      this.error('supabase', `Supabase Error: ${message}`, error, data, metadata)
    },
    auth: (message: string, data?: any, metadata?: Partial<LogEntry>) => {
      this.info('auth', `Auth: ${message}`, data, metadata)
    },
    rls: (message: string, data?: any, metadata?: Partial<LogEntry>) => {
      this.warn('supabase', `RLS: ${message}`, data, metadata)
    }
  }
