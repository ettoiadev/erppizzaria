/**
 * Script para testar a conexão com o Supabase (versão TypeScript)
 * 
...
function printTestResult(name: string, success: boolean, details: string | null = null): void {
  const icon = success ? '✅' : '❌';
...
async function testSupabaseConnection(): Promise<void> {
  ...
  try {
    ...
    const { count, error: pingError } = await supabase.from('profiles').select('id', { count: 'exact', head: true });
    ...
    printTestResult('Usuário admin', !adminError && adminData && adminData.length > 0, 
      ...
      (adminData && adminData.length > 0) ? 
        `Admin encontrado: ${(adminData[0] as AdminUser).email} (${(adminData[0] as AdminUser).full_name})` : 
